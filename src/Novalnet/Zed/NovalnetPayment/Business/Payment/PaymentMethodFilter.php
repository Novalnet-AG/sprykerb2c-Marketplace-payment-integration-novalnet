...
class PaymentMethodFilter implements PaymentMethodFilterInterface
{
    ...
    protected function isAvailable(PaymentMethodTransfer $paymentMethodTransfer, QuoteTransfer $quoteTransfer, $enabledPayments): bool
    {
        ...
        $merchantDetails = [];
        $subVendors = $standardParameter->getSubVendors();
        foreach ($quoteTransfer->getItems() as $itemTransfer) {
            if ($itemTransfer->getMerchantReference() && isset($subVendors[$itemTransfer->getMerchantReference()])) {
        ...
        if (
            ...
            && !in_array($paymentMethod, ['novalnetCreditCard', 'novalnetSepa', 'novalnetSepaGuarantee', 'novalnetInvoice',
            'novalnetInvoiceGuarantee', 'novalnetPrepayment', 'novalnetIdeal', 'novalnetSofort', 'novalnetGiropay',
            'novalnetPrzelewy', 'novalnetEps', 'novalnetPostfinanceCard', 'novalnetPostfinance', 'novalnetBancontact'])
        ) {
            $flag = false;
        }
