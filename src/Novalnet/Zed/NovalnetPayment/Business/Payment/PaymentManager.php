...
class PaymentManager implements PaymentManagerInterface
{
    ...
    protected function setSubVendorData(OrderTransfer $orderTransfer, &$requestData)
    {
        ...
        foreach ($merchantDetails as $merchantReference => $totalAmount) {
            if (isset($config[$merchantReference])) {
                $subVendors[$config[$merchantReference]['vendor_id']] = $totalAmount;
            } else {
                if (isset($subVendors['amount'])) {
                    $subVendors['amount'] += $totalAmount;
                } else {
                    $subVendors['amount'] = $totalAmount;
                }
            }
    ...
    protected function setTransactionData(OrderTransfer $orderTransfer, &$requestData)
    {
        ...
        $transactionData = [
            ...
            'system_version' => '202108.0-NN1.0.1',
        ];

        $subVendors = $this->setSubVendorData($orderTransfer, $requestData);

        if (!empty($subVendors)) {
            if (!empty($subVendors['amount'])) {
                $transactionData['amount'] = $subVendors['amount']; // If mixed cart involved
                unset($subVendors['amount']);
            } else {
                $transactionData['amount'] = 0; // only subvendors product available in the cart
            }
    ...
    public function refund(NovalnetRefundTransfer $refundTransfer)
    {
        ...
        foreach ($merchantDetails as $merchantReference => $refundAmount) {
            $tid = !empty($config[$merchantReference])
                ? $paymentResponse[$config[$merchantReference]['vendor_id']]['tid']
